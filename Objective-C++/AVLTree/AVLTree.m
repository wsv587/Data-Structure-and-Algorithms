 ...
 * 如果失衡也是添加的叶子节点导致的, 所以从添加的叶子节点开始递归向上查找失衡节点, 直到根节点
 * 找到失衡节点进行左旋转、右旋转操作
...
- (void)afterRemove:(AVLNode *)node {
    while ((node = (AVLNode *)node.parent)) {
        if ([self isBalance:node]) {
            [node updateHeight];
        } else {
            [self rebalance:node];
            // 恢复平衡后不能退出，需要继续看父节点是否失衡，直到root节点
        }
    }
 ...
 * node: 失衡节点 也就是g节点
 如何判断LL、RR、LR、RL？
 1.先判断g节点（失衡节点）是L还是R?
 想知道是L、R其实就看g节点（也就是失衡节点）的失衡因子就知道了。即g节点平衡因子 = g节点的左子树的高度 — g节点右子树的高度。平衡因子为正数说明左子树高属于L，否则右子树高属于R.
